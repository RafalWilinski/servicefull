...
So you need to add or change a query access pattern. Basically, there are three options is such a scenario:

1. Your table structure is sufficient and data allows querying it in such a way
2. Your table structure is sufficient but some records don't have required data
3. Your table structure is insufficient

While the first scenario is painless and the third requires remodeling your table, the second scenario is not that bad. It just requires migrating a portion of your data in the table. For example, some records need to be queried in a different way but these records do not have the HashKey of the desired Index set. What probably you need to do, is only copy the other attribute in place of desired attribute or aggregate it from other record parameters.
...
- Your records in the database probably include `pk`, `sk`, `data` while your models probably have attributes like `id`, `productId` etc. Some mapping function here was really helpful for us.
...
Even though I haven't used [Contributor Insights](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/contributorinsights_HowItWorks.html) in the mentioned project because it has been just announcement as a part of pre re:Invent release hype train, I gave it a shot in my side-project. This diagnostic tool is definitely helpful in performance tuning and finding bottlenecks by finding the most frequently accessed partiton keys.
...
What's been also helpful was [Service Map](https://docs.aws.amazon.com/xray/latest/devguide/xray-console.html#xray-console-servicemap). Just a glance at it showed us that our problem lies in custom authorizer function which fetched too much data and caused whole the API to slow down.
...
You might say - let's do that synchronically. That's not a good idea. This probably would take too much time inside Lambda and [should be rather distributed](https://servicefull.cloud/blog/dynamodb-mass-update/).
...
- Database returned fewer data so our API returned fewer data too, making everything faster and more responsive
