...
Actually, this step is not just purely engineering task, but it blends with business and product competences. That's why I think you should definitely participate in the planning session as soon as possible to understand business use cases better and propose developer-friendly solutions. Clear requirements equals to happy developer(s) equals product delivered and deadline met.
...
What's been also helpful was [Service Map](https://docs.aws.amazon.com/xray/latest/devguide/xray-console.html#xray-console-servicemap). Just a glance at it showed us that our problem lies in custom authorizer function which fetched too much data and caused slowdown to the whole API.
