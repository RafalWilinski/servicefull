...
length: 8 minutes
image: https://s3.amazonaws.com/servicefull.cloud/images/template-servicefull.png
---
...
## Planning your query access patterns is absolutely essential
...
While first scenario is painless and third requires remodeling your table, second scenario is not that bad. It just requires migrating a portion of your data in the table. For example, some records need to be queried in different way but these records do not have the HashKey of desired Index set. What probably you need to do, is only copy the other attribute in place of desired attribute or aggregate it from other record parameters.

If you're still in development, writing a simple _get and for each record run update_ script might be sufficient, for production databases or larger datasets consider using [mass update pattern](https://servicefull.cloud/blog/dynamodb-mass-update/) which I've described in my other blogpost.

## Invest some time in proper abstractions and create your own opinionated ORM

As you start writing some code persistence layer code, you'll immediately notice that many things are repeating and most API calls require complex parameters like `UpdateExpression` or `ExpressionAttributeValues`. These should be definitely abstracted away into simpler function signatures.

Here are some ideas:

- Each call will require `TableName` (exactly the same with single-table design) and `IndexName`. Writing that into each call is unnecessary duplication
- You should always use `Limit` (more on that later) and probably you should also restrict that number to some upper bound like 1000. Something like `Math.max(params.limit, 1000)` could be also there
...
- Your records in database probably include `pk`, `sk`, `data` while your models probably have attributes like `id`, `productId` etc. Some mapping function here was really helpful for us.
...
Even though I haven't used [Contributor Insights](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/contributorinsights_HowItWorks.html) in the mentioned project, I gave it a shot in my side-project. This diagnostic tool is definitely helpful in performance tuning and finding bottlenecks.
...
What's been also helpful was [Service Map](https://docs.aws.amazon.com/xray/latest/devguide/xray-console.html#xray-console-servicemap). Just a glance at it showed us that our problem lies in custom authorizer function which fetched too much data and caused whole API to slow down.
...
Imagine a scenario where you have a system with entity `Group` which can have many `Members`. Members are linked to the group using separate records where `pk: Group-ID` and `sk: Member-ID`. In such scenario in SQL world, `FOREIGN KEYS` and `ON DELETE CASCADE` would guarantee that once "Group" is deleted, all "membership" records would be deleted to.

Unfortunately, that's not the case in Dynamo.

You might say - let's do that synchronically. That's not a good idea. First, each
...
- Less garbage in the table and less referential integity issues
...
At the beginning we've made this mistake - we started storing avatar blobs inside `avatar` attribute. This wasn't a good idea. As single query only returns a result set that fits within the 1 MB size limit, we sometimes saw some calls returning only 3 items effectively making everything slower than it should be. Solving this problem was fortunately quite easy - we uploaded all blobs to S3 and just saved the link referencing it in the table.

## Always use `Limit` in queries and paginate

In our case, DynamoDB served the purpose of persistence layer for community platform API. In use case like that, according to our analyses, users are most likely to be interested in latest 10 announcements or message in channel. If they would like to see more, they could just request it and application would get them these using pagination. This approach of returning minimal amount of data at first gave us a range of benefits:

- Database returned less data so our API returned less data too making everything faster and more responsive
- Because there was less data returned, our Lambdas also needed less time to process it so we paid even less for the compute part of Serverless stack
...
Spent some time [reading the docs about limitations](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/Limits.html), seriously. It would save us many frustrations. Some things that were surprising to us include:
...
- Attribute value cannot be an empty string
- Maximum item size is 400 KB

## Always use `ExpressionAttributeNames`

There are so many [reserved keywords](https://docs.aws.amazon.com/amazondynamodb/latest/developerguide/ReservedWords.html) in DynamoDB, that instead of guessing if it's reserved and what's not, assume every attribute name is reserved. Of course, I would not recommend doing that manually with each database call. Instead, this should be hidden from developers by your _something-like-ORM_ abstraction.
