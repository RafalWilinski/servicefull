...
In V2, once we had our `DocumentClient` constructed, interaction with DynamoDB in Node.js was boiling down to using [methods like `get`, `put`, `scan` or `query`](https://dynobase.dev/dynamodb-nodejs/).
...
In V3, `DocumentClient` is imported from a different package and created using `.from()` method.
