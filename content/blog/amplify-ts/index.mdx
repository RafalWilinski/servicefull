...
Lastly, we need to install `typescript` as `devDependency` at the root of our project and tell Amplify what to do with our TS-based function. Fortunately, recently Amplify added [Build Options for Functions]([https://aws-amplify.github.io/docs/cli-toolchain/usage#functions](https://aws-amplify.github.io/docs/cli-toolchain/usage#functions)). With that in mind, we can add following command to `package.json`:
```
"scripts":  {
...
You can test it locally using `amplify function invoke <YOUR_FN_NAME>` (make sure it's built first!). 
